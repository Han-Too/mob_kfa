<x-app-layout>
    ...
    <div class="card">
        <div class="card-header border-0 pt-6">
            ...
            <div class="card-title d-block">
                ...
                <h4 class="fw-bold fs-6 mb-2 mt-2 text-muted">Management of {{ $merchant->nama_merchant }} features</h4>
            </div>
            <!--begin::Card title-->
        </div>
        <!--begin::Card header-->
            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_privileges">
                ...
                <thead>
                    ...
                    <tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                        <th class="min-w-125px">Fitur Pembayaran</th>
                        <th class="min-w-100px">Type</th>
                        <th class="min-w-100px">Application Date</th>
                        <th class="min-w-125px text-center">Status Applicant</th>
                        <th class="min-w-100px">Updated By</th>
                        <th class="text-center min-w-100px">Status Internal</th>
                    </tr>
                    <!--end::Table row-->
                </thead>
                <!--end::Table head-->
                    ...
                    @foreach ($data as $item)
                        <tr>
                            ...
                            <td class="align-items-center" style="vertical-align: top">
                                ...
                                <div class="">
                                    <a href="{{ route('applicant.show', $item->token_applicant) }}"
                                        class="text-gray-800 text-hover-primary mb-1 d-flex flex-column">{{ $item->payment }}</a>
                                </div>
                                <!--begin::User details-->
                            </td>
                            <!--end::User=-->
                            ...
                            <td style="vertical-align: top">
                                {{ $item->merchant->pengajuan_sebagai }}
                            </td>
                            ...
                            <td style="vertical-align: top">
                                <div class="d-flex align-items-center mb-7">
                                    ...
                                    <div class="flex-grow-1 me-2">
                                        <a href="#"
                                            class="fw-bolder text-gray-800 text-hover-primary fs-6">{{ App\Helpers\Utils::getUserName($item->updated_by) }}</a>
                                        <span class="text-muted fw-bold d-block">{{ App\Helpers\Utils::getUserRole($item->updated_by) }}</span>
                                    </div>
                                    <!--end::Title-->
                                </div>
                            </td>
                            ...
                            <td style="vertical-align: top" class="text-end">
                                <span
                                    class="d-flex flex-column badge badge-lg badge-light-{{ App\Helpers\Utils::badgeApplicant($item->internal_status) }}">{{ App\Helpers\Utils::internalPaymentStatusByRecomendation($item->internal_status) }}</span>
                                @if ( $item->layer_id == App\Helpers\Utils::getLayerIdByRole(Auth::user()->role_id))
                                    <a href="{{ url('/applicants/payments/show/'.$item->id.'/'.$item->merchant->token_applicant) }}"
    ...
    <!--end::Card-->
    @section('scripts')
        <script src="{{ asset('tadmin/js/custom/applicant/payments/index.js') }}"></script>
        <script>
